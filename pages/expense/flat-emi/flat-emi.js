...
(function () {
  ...
  function paymentLabel(value) {
    return ({ emi: 'Bank EMI', downpayment: 'Downpayment', bullet: 'Bullet Payment', 'personal-loan': 'Personal Loan', 'friend-repayment': 'Friend / Personal Repayment' })[value] || value;
  }
  ...
  async function importExcel(file) {
    ...
    const requiredHeaders = ['TXN-DATE', 'AMOUNT', 'EMI / BULLET POINT', 'CHECK NUMBER'];
    ...
    rows.forEach((row, index) => {
      ...
      const marker = String(headerValue(row, 'EMI / BULLET POINT') || '').trim();
      if (!normalizedDate || !Number.isFinite(amount) || amount <= 0 || !/^(emi|bullet)$/i.test(marker)) {
        errors.push(`Row ${index + 2}: date, positive amount, and EMI/BULLET value are required`);
        return;
      }
      imported.push({ _id: idFor('payment'), type: 'flat-emi-payment', date: normalizedDate, paymentType: normalizePaymentType(marker), amount, note: headerValue(row, 'CHECK NUMBER') ? `Check: ${headerValue(row, 'CHECK NUMBER')}` : 'Imported from Excel' });
    });
